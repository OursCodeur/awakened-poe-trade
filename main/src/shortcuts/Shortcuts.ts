...
function pressKeysToCopyItemText (pressedModKeys: string[] = [], showModsKey: string, keyboard: Keyboard) {
  ...
  if (process.platform !== 'darwin' && !isPlasmaWayland) {
    // On X11, don't toggle keys that are already being pressed. Wayland's
    // virtual keyboard does not inherit modifiers from the physical keyboard.
    //
